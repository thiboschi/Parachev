...
mod tests {
    ...
    #[test]
    fn test_affaire_avec_goujons() {
        let resultat = extraire_goujons_fc_gouj("1100719879.xlsx")
            .unwrap()
            .expect("la feuille FC-GOUJ doit exister");
        ...
        assert_eq!(resultat.detail[0].profil, "HLM 1000");
        assert_eq!(resultat.detail[0].nb_goujons, 585.0);
    }

    #[test]
    fn test_affaire_sans_goujons_reels_mais_feuille_presente() {
        let resultat = extraire_goujons_fc_gouj("1100706839.xlsx")
            .unwrap()
            .expect("la feuille FC-GOUJ existe même si vide");

        assert_eq!(resultat.nb_barres, 23);
        assert_eq!(resultat.nb_goujons_total, 0.0);
    }

    #[test]
    fn test_affaire_placeholder_zero_avant_ref() {
        let resultat = extraire_goujons_fc_gouj("1100546190.xlsx")
            .unwrap()
            .expect("la feuille FC-GOUJ existe même si vide");

        assert_eq!(resultat.nb_barres, 39);
        assert_eq!(resultat.nb_goujons_total, 0.0);
    }

    #[test]
    fn test_affaire_sans_feuille_goujonnage() {
        let resultat = extraire_goujons_fc_gouj("1100662668.xlsx").unwrap();
